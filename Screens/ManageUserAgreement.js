...
const ManageUserAgreement = ({ navigation }) => {
  ...
  const saveUserAgreement = () => {
    ...
    const userAgreementRef = ref(db, 'userAgreement'); // Replace with your Firebase Realtime Database reference
  ...
  useEffect(() => {
    ...
    onValue(userAgreementRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        setUserAgreement("User Agreement not found.");
      }
    });
...
export default ManageUserAgreement;
