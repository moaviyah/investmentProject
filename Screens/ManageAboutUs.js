...
const ManageAboutUs = ({ navigation }) => {
  const [userAgreement, setUserAgreement] = useState("Loading About us...");

  // Function to save the edited User Agreement
  const saveUserAgreement = () => {
    // Get a reference to the Firebase Realtime Database
    const db = getDatabase();
    const userAgreementRef = ref(db, 'aboutus'); // Replace with your Firebase Realtime Database reference
  ...
  useEffect(() => {
    ...
    onValue(userAgreementRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        setUserAgreement("About us not found.");
      }
    });
