...
const UserAgreement = ({ navigation }) => {
  const [aboutUsContent, setAboutUsContent] = useState("Loading User Agreement...");

  // Function to fetch and update "About Us" content from Firebase
  useEffect(() => {
    const db = getDatabase();
    const aboutUsRef = ref(db, 'userAgreement'); // Replace with your Firebase Realtime Database reference
    ...
    onValue(aboutUsRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        setAboutUsContent("User Agreement not found.");
      }
    });
